...
The client uses an IPv4 TCP socket. it creates a stream socket (AF_INET,
SOCK_STREAM), connects to 127.0.0.1:8001, and sends data using write calls over
a TCP connection.

2. What data is the client sending to the server?
The client sends 5 fixed-size messages taken from the messages[] array. Each is
copied into a 1024 byte buffer with strncpy

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
receives a pointer to a struct acceptor_args. Struct has an atomic bool run flag
to cotnrol when acceptor should stop, pointer to shared list_handle where
messages are stored, pointer to the mutex to protect list. The acceptor thread
uses this argument to access shared state and to know when to stop accepting
clients.

2. How are received messages stored?
Messages are copied into dynamically allocated memory, put inside list_node. and
appened to a singly linked list.

3. What does `main()` do with the received messages?
Main waits until total number of messages in the list is at least MAX_CLIENTS *
NUM_MSG_PER_CLIENT then prints and frees them

4. How are threads used in this sample code?
 One acceptor thread is created, and each connected client is handled by its own
thread.
*/
...
/*
Answers to non-blocking sockets
1. Sockets are made non-blocking using fcntl(fd, F_SETFL, flags | O_NONBLOCK).
2. The listening socket and each accepted client socket are set to non-blocking.
3. This prevents accept() and read() from blocking forever, allowing threads to
keep looping and shut down cleanly.
*/
