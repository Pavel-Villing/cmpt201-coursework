...
It is TCP since the socket is created with SOCK_STREAM and the client calls
connect()

3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code?

It reads data from the keyboard standard input. We can tell from the while loop
while ((num_read = read(STDIN_FILENO, buf, BUF_SIZE)) > 1) in client.c that uses
STDIN_FILENO to store user input in a buffer and send using write

4. How does the client program end? How can you tell that in the code?
The client ends when STDIN returns a value <=1. we can tell since that would
break us out of the while loop, then after the if statement if num_read ==-1 the
client closes the socket and client exits
...
int main() {
  ...
  for (;;) {
    ...
    pthread_t tid;
    int err = pthread_create(&tid, NULL, handle_client, info);
    if (err != 0) {
      errno = err;
