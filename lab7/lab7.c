...
int main(void) {
  ...
  IntermediateInput *midList =
      (IntermediateInput *)malloc(sizeof(IntermediateInput) * MAX_ITEMS);
  ...
  for (int i = 0; i < rawCount; i++) {
    ...
    if (found == -1) {
      ...
      groups[groupCount].lines =
          (int *)malloc(sizeof(int) * groups[groupCount].cap);
      groups[groupCount].lines[groups[groupCount].count++] = ln;
      groupCount++;
    } else {
      if (groups[found].count == groups[found].cap) {
        groups[found].cap *= 2;
        groups[found].lines = (int *)realloc(groups[found].lines,
                                             sizeof(int) * groups[found].cap);
      }
